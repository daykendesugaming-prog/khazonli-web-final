    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md animate-fade-in">
      <div className="bg-[#121826] border border-[#00A8FF]/30 w-full max-w-md rounded-[30px] p-8 shadow-[0_0_50px_rgba(0,168,255,0.1)] relative overflow-hidden">
        ...
        <h3 className="text-2xl font-black text-white mb-1 uppercase tracking-tighter">
          Vender en <span className="text-[#00A8FF]">{selectedServer.server}</span>
        </h3>
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div>
            <label className="text-[10px] font-black text-gray-600 uppercase ml-1">¿Cuántos millones vendes?</label>
            <input 
              type="number" 
              className="w-full bg-[#0B0F19] border border-gray-800 rounded-xl p-4 text-white outline-none focus:border-[#00A8FF]/50 mt-1 transition-all"
              placeholder="Ej: 50"
          ...
          <div>
            <label className="text-[10px] font-black text-gray-600 uppercase ml-1">Método de recepción</label>
            <select 
          ...
          <div>
            <label className="text-[10px] font-black text-gray-600 uppercase ml-1">Tus datos bancarios / Pago móvil</label>
            <textarea 
          ...
          <div>
            <label className="text-[10px] font-black text-[#00A8FF] uppercase ml-1">WhatsApp de contacto *</label>
            <input 
              type="tel" 
          ...
          {toNumber(amount) > 0 && (
            <div className={`p-5 rounded-2xl mt-6 border transition-all duration-500 ${
              paymentMethod === 'Binance Pay' 
              ...
              <p className="text-gray-400 text-[10px] font-black uppercase mb-1 tracking-widest text-center">
                Recibirás exactamente:
              </p>
              <p className={`text-3xl font-black text-center ${
                paymentMethod === 'Binance Pay' ? 'text-[#FBB03B]' : 'text-[#00A8FF]'
              }`}>
                {paymentMethod === 'Binance Pay' 
                  ? `${totalUsd.toFixed(2)} USDT` 
                  : `${formatBs(totalBs)} Bs.`}
