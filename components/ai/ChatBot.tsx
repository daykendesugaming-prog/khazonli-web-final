...
export default function ChatBot({ mode = 'bubble' }: { mode?: 'full' | 'bubble' }) {
  ...
  const ChatWindow = (
    <div className={`flex flex-col bg-[#121826]/95 backdrop-blur-xl border border-gray-800 shadow-2xl rounded-[30px] overflow-hidden animate-fade-in ${
      mode === 'bubble' ? 'fixed bottom-24 right-6 w-[350px] md:w-[380px] h-[550px] z-[100]' : 'w-full h-[600px] relative'
    }`}>
      {/* HEADER DEL CHAT */}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar bg-[#0B0F19]/50">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
            <div className={`max-w-[85%] p-4 rounded-[20px] text-[13px] leading-relaxed shadow-lg ${
              msg.role === 'user' 
      ...
      <>
        {!isOpen && (
          <button 
            ...
            className="fixed bottom-6 right-6 w-16 h-16 bg-[#0B0F19] rounded-full shadow-[0_0_25px_rgba(0,168,255,0.5)] flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-[100] border-2 border-[#00A8FF]/50 p-0 overflow-hidden group"
          >
